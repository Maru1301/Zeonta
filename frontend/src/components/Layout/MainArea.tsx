...
export default function MainArea({
  ...
  const slot1Top = slot1HasTabs ? TAB_BAR_HEIGHT : 0
  ...
  return (
    <Box sx={{ flex: 1, overflow: 'hidden', position: 'relative' }}>
      ...
      {!slot0HasTabs && (
        <Box sx={{ position: 'absolute', top: 0, bottom: 0, left: 0, width: slot0Px, overflow: 'auto' }}>
          <EmptyState />
        </Box>
      )}
      {splitEnabled && !slot1HasTabs && (
        <Box sx={{ position: 'absolute', top: 0, bottom: 0, left: slot1Left, width: containerWidth - slot1Left, overflow: 'auto' }}>
          <EmptyState />
        </Box>
      )}
    </Box>
